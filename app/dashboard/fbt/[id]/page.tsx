...
const FrequentlyBoughtTogether = () => {
    ...
    useEffect(() => {
        ...
        const fetchProduct = async ()=>{
            ...
            const fbt = product?.FBT
            setFbtProducts(fbt as any)
    ...
    const searchPrimaryProduct = (theSearchQuery: string) => {
        ...
        const results: any = products.filter(product => product.name.toLowerCase().includes(theSearchQuery.toLowerCase())).slice(0, 5)
    ...
    const handleSaveAndContinue = async (productId: string) => {
        ...
        const frequentlyBoughtIds = fbtProducts.filter(item => item._id).map(item => item._id)
        console.log(fbtProducts);
        
        console.log(frequentlyBoughtIds);
        

        try {
            const response = await fetch(`https://mkhasa-bfdb6fabd978.herokuapp.com/api/v1/FBT/66542da6f1f73b11f47b728a/${productId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'  // Specify that the request body is JSON
                },
                body: JSON.stringify({ FBT: frequentlyBoughtIds })
            });

            if (!response.ok) throw new Error('Failed to update frequently bought product');
    ...
    const handleAddProduct = (productId: string) => {
        ...
        if (newProduct) {
            setFbtProducts((prevProducts: any) => {
    ...
    const handleRemoveProduct = (productId: string) => {
        let newProducts: any = fbtProducts.filter((product: any) => product._id !== productId)
        setFbtProducts(newProducts)
    };
    ...
    return (
        <div className='max-w-[1140px] m-auto'>
                                        ...
                                        <div className=' gap-4 justify-between items-center bg-gray-50 p-2'>
                                            <div className='flex flex-1 gap-4 sm:gap-2 items-center justify-between'>
                                                <div className='w-2/4 max-w-[70px] bg-gray-100 hidden sm:block'>
                                                    <Image src={primaryProduct.mainImage} alt='Selected product' className='w-full' />
                                                </div>
            ...
            {/* ADD FREQUENTLY BOUGHT PRODUCT */}
            {
                primaryProduct && (
                    <div className='mb-8'>
                        <h2 className="font-semibold text-center md:text-left bg-gray-50 py-4 pl-2 mb-6 text-xl">Add Frequently Bought Product</h2>
                        <form className="w-full md:flex-grow lg:max-w-[50%] relative mb-8 lg:mt-0 search-form" onSubmit={(e) => e.preventDefault()}>
                                ...
                                <ul className={`absolute top-[2.8rem] left-0 z-40 rounded bg-white shadow-md p-4 border w-full search-result overflow-y-scroll max-h-80 ${!searchQuery && 'hidden'}`}>
                                    {
                                        filteredProducts?.length > 0 ?
                                            filteredProducts?.map((ele, index) => {
                                                const isProductChecked = fbtProducts.some(product => product._id === ele._id);
                                                return (
                            ...
                            <>
                                {
                                    fbtProducts.length > 0 ?
                                        ...
                                        <div className='py-16 text-center bg-gray-50 text-gray-400'>
                                            <p>This product has not FBT products</p>
                                        </div>
                                }
                            </>
                        }
