...
import { Dialog, DialogTrigger, DialogContent, DialogOverlay, DialogTitle, DialogClose } from './ui/dialog';
...
const OrderTable = () => {
    ...
    const riderNameRef = useRef<HTMLInputElement | null>(null);
    const riderMobileRef = useRef<HTMLInputElement | null>(null);
    ...
    const fetchOrders = async (adminId: string) => {
        setIsLoading(true);
        try {
            const response = await fetch(`/api/proxy?path=all/order/system/${adminId}`);
            if (!response.ok) {
    ...
    const handleDispatch = async (orderId: String) => {
        ...
        const ridersMobile = null // riderMobileRef.current.value
        const ridersName = null // riderNameRef.current.value
        try {
            const response = await fetch(`https://mkhasa-bfdb6fabd978.herokuapp.com/api/v1/dispatch/order/${session?.user._id}/${orderId}`, {
                body: JSON.stringify({ ridersName, ridersMobile }),
                method: 'POST'
            })
            if (!response.ok) {
                throw new Error('Unable to dispatch order');
            }
            const data = await response.json()
            console.log(data);
    ...
    const handleDeliver = async (orderId: String) => {
        setLoadingOrder(true)
        try {
            const response = await fetch(`https://mkhasa-bfdb6fabd978.herokuapp.com/api/v1/deliver/order/${session?.user._id}/${orderId}`, { method: 'POST' })
            if (!response.ok) {
                throw new Error('Unable to deliver product. Try again.')
            }
            const data = await response.json()
            console.log(data)
        } catch (error) {
                        ...
                        <TableRow key={order._id} className='bg-gray-100'>
                            <TableCell className='pr-10 lg:pr-0'>
                                <div className="flex items-center">
                                    ...
                                    /> */}
                                    <div>
                                        <div className='text-xs line-clamp-1 lg:text-base'>{order.name}</div>
                            ...
                            <TableCell className='text-xs lg:text-base'>{order?.date?.toLocaleString()}</TableCell>
                            <TableCell className='text-xs lg:text-base'>₦{order.total.toLocaleString()}</TableCell>
                            <TableCell className='text-xs lg:text-base'>{order.payment && order.status == 'pending' ? 'Paid' : !order.payment && order.status == 'pending' ? 'Pending' : order.status}</TableCell>
                                            ...
                                            <>
                                                {
                                                    order.status === "Pending" ?
                                                        ...
                                                        <Dialog>
                                                            <DialogTrigger>
                                                                <Button disabled={loadingOrder}>Dispatch</Button>
                                                            </DialogTrigger>
                                                            ...
                                                            <DialogContent>
                                                                ...
                                                                <div className='flex-col gap-3'>
                                                                    <div className='mb-4'>
                                                                        ...
                                                                        <Input ref={riderNameRef} />
                                                                    </div>
                                                                    <label className='font-bold'>
                                                                        Rider's Phone Number
                                                                        <Input type='number' ref={riderMobileRef} />
                                                                    </label>
                                                                </div>
                                                        ...
                                                        <Button disabled={loadingOrder} onClick={() => { handleDeliver(order._id) }}>Deliver</Button>
                                                }
                                            </>
                                        )
                                    }
