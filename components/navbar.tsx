...
const AdminNavbar: React.FC<AdminNavbarProps> = ({ toggleMobileSidebar }) => {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!value) return;
    router.push(`/dashboard/search?s=${value}`);
  };
  ...
  return (
    <Wrapper>
      <nav className="bg-white flex flex-wrap items-center justify-between gap-x-4 py-2">
        ...
        <form onSubmit={onSubmit} className="w-full md:flex-grow lg:max-w-xl relative flex items-center gap-4 p-2 rounded-[50px] mt-3 lg:mt-0 bg-gray-100">
          <input
            id="search"
            name="search"
            type="text"
            placeholder="search product"
            className="w-full px-6 rounded-full outline-none bg-transparent"
          />
          <button
            aria-label="search for product"
            className="rounded-[50px] block bg-black text-white text-sm p-1 px-2  "
            type="submit"
          >
            Search
          </button>
